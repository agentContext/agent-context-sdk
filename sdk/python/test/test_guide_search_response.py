# coding: utf-8

"""
    Agent App & Guide Public API

    基本地址：`/v1/`（已在 servers 配置） - 普通接口 **无鉴权** - 统一错误：`{\"error\":\"reason\"}` - 分页：`page` 默认 1，`limit` 默认 20 - `score` 为相似度（仅 /guide/search/{pkgname} 返回） 

    The version of the OpenAPI document: 1.0.0
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from agent-context-sdk.models.guide_search_response import GuideSearchResponse

class TestGuideSearchResponse(unittest.TestCase):
    """GuideSearchResponse unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional) -> GuideSearchResponse:
        """Test GuideSearchResponse
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `GuideSearchResponse`
        """
        model = GuideSearchResponse()
        if include_optional:
            return GuideSearchResponse(
                data = [
                    agent-context-sdk.models.guide.Guide(
                        id = 56, 
                        package_name = '', 
                        vector = null, 
                        action = '', 
                        description = '', 
                        tokens = 56, 
                        source = '', 
                        solution = '', )
                    ],
                paging = agent-context-sdk.models.paging.Paging(
                    limit = 56, 
                    page = 56, 
                    total = 56, 
                    kw = '', ),
                score = [
                    1.337
                    ]
            )
        else:
            return GuideSearchResponse(
                data = [
                    agent-context-sdk.models.guide.Guide(
                        id = 56, 
                        package_name = '', 
                        vector = null, 
                        action = '', 
                        description = '', 
                        tokens = 56, 
                        source = '', 
                        solution = '', )
                    ],
                paging = agent-context-sdk.models.paging.Paging(
                    limit = 56, 
                    page = 56, 
                    total = 56, 
                    kw = '', ),
                score = [
                    1.337
                    ],
        )
        """

    def testGuideSearchResponse(self):
        """Test GuideSearchResponse"""
        # inst_req_only = self.make_instance(include_optional=False)
        # inst_req_and_optional = self.make_instance(include_optional=True)

if __name__ == '__main__':
    unittest.main()
